...
>
> **实现阶段审查与 impl-verify 的关系**：使用 `subagent-driven-development` 时，其自带的 per-task spec review + code quality review 提供即时反馈，避免错误级联到后续 Task。impl-verify 在实现全部完成后做更高层次的需求/领域知识/Plan/工程标准交叉验证。两者互补，不互相替代。实现策略选择规则见 risk-classifier 的"实现策略选择"章节。
>
