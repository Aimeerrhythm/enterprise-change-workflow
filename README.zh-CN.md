...
| `ecw:impl-verify` | 实现完成后（P0-P2） | 多轮收敛：代码 ↔ 需求/规则/Plan/工程标准，严重度分级退出。替代 code-reviewer |
...
| 组件 | 调度方 | 说明 |
|------|--------|------|
| `biz-impact-analyzer` | `ecw:biz-impact-analysis` | 5 步分析：Diff 解析 → 依赖图查询 → 代码扫描 → 外部系统评估 → 报告生成 |
...
| 组件 | 触发方式 | 说明 |
|------|---------|------|
| `verify-completion` | PreToolUse 自动拦截 TaskUpdate(completed) | 4 项硬拦截 + 1 项定向提醒 |
...
│   └── verify-completion.py     # 完成验证 Hook（5 项检查：4 项硬拦截 + 1 项定向提醒）
├── templates/                   # 配置和知识文件模板
│   ├── ecw.yml                  # 项目配置模板
...
- **superpowers plugin** — 提供 `writing-plans`、`executing-plans`、`systematic-debugging` 等基础 skill，ECW 的多个环节依赖这些 skill
