...
| `ecw:workspace` | 仅手动（`/ecw:workspace`） | **多仓库工作区**：git worktree 隔离 + 6 Phase 协调器（业务拆分 → 各服务代码分析 → 契约对齐 → 子 session 自拆任务+并行实施 → 跨服务验证 → 推送）。子 session 拥有任务拆分权，Coordinator 负责契约对齐，原始需求原文下发防止失真。 |
