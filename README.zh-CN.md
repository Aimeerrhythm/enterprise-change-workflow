...
参见 [docs/design-principles.md](docs/design-principles.md) — 核心思想：每一层脚手架都必须通过「模型升级测试」。放大器因模型变强而增值，拐杖因模型变强而成为死重。
...
当变更涉及 **2 个以上独立仓库**（如 Provider + Consumer 服务）时，使用 `ecw:workspace` 代替标准单仓库流程。工作区协调器负责业务拆分、契约对齐和多 session 并行实施。

**适用场景**：跨仓库接口依赖变更（Dubbo、MQ）。**不适用**：单仓库多模块变更（用标准流程）、单体仓库内多服务（用 `ecw:domain-collab`）。

```
/ecw:workspace create service-a ../service-b
        |
  Pre-flight — 各服务 ECW 就绪检查
        |
  Phase 1 — 业务拆分（Coordinator 执行，不读代码）
        |     各服务角色（Provider/Consumer）、交互模式
        |     产出：cross-service-plan.md + 各服务 workspace-analysis-task.md
        |     [用户确认职责划分]
        |
  Phase 2 — 各服务代码分析（子 session 并行）
        |     各服务打开终端分屏，独立分析代码
        |     产出：各服务 analysis-report.md
        |
  Phase 3 — 契约对齐 + 冲突解决
        |     交叉验证：MQ Topic/DTO、Dubbo 签名、归属权
        |     冲突 → 用户决策
        |     产出：各服务 confirmed-contract.md
        |
  Phase 4 — 各服务实施（子 session）
        |     Layer 1：Provider 服务先完成 + 发布 API Jar
        |     Layer 2：Consumer 服务并行实施
        |     各自运行：ecw:writing-plans → 实现 → ecw:impl-verify
        |     产出：各服务 status.json
        |
  Phase 5 — 跨服务验证
        |     Dubbo 签名一致性、MQ DTO 兼容性、Maven 版本对齐
        |
  Phase 6 — 汇总 & 推送
              聚合变更、确认部署顺序、批量推送
```
...
| 命令 | 说明 |
|------|------|
| `create <services...>` | 创建工作区（git worktree 隔离），自动启动 run |
| `run "<requirement>"` | 执行 6 Phase 协调器流程 |
| `status` | 查看所有服务的 git + session 状态 |
| `push` | 逐服务确认后批量推送 |
| `destroy` | 清理 worktree + 工作区目录 |

**核心设计原则：**
- **Phase 1 不读代码** — 业务拆分在代码分析之前完成，避免过早优化
- **原始需求原文下发** — 不改写需求描述，防止意图失真
- **子 session 自治** — 各服务自行拆分任务，Coordinator 只负责契约对齐
- **架构决策用户把关** — 契约冲突、交互模式、推送确认均需用户决策
...
| `ecw:impl-orchestration` | Plan 执行（4+ Tasks，P0/P1） | 依赖图分层并行 subagent + worktree 隔离 + 风险感知 review |
...
| `ecw:knowledge-audit` | 手动，定期执行 | 知识库健康度审计：失效引用、三条件合规、内容构成分析 |
| `ecw:knowledge-track` | 手动，任务后执行 | 文档利用追踪（hit/miss/redundant/misleading），量化知识库 ROI |
| `ecw:knowledge-repomap` | 手动，重构后执行 | 从 ecw.yml component_types 自动生成代码结构索引 |
| `ecw:workspace` | 仅手动（`/ecw:workspace`） | **多仓库工作区**：git worktree 隔离 + 6 Phase 协调器（Phase 1 纯业务拆分禁止读代码 → 各服务知识优先代码分析 → MQ/Dubbo 分场景契约对齐与分派 → 并行实施 → 跨服务字段/签名深度验证 → 推送）。子 session 使用 bypassPermissions，拥有任务拆分权；MQ 场景契约确认后并行，Dubbo 串行等 Layer 1 完成。 |
...
| `config-protect` | minimal, standard, strict | 阻止 AI 修改 ECW 关键配置文件（ecw-upgrade/ecw-init 通过 marker 文件自动放行） |
...
ECW 当前支持 **Java/Spring** 项目。`ecw.yml` 中的 `component_types` 和 `scan_patterns` 均按 Java 约定配置：
...
│   ├── impl-verify/             # 实现正确性验证（多轮收敛）
│   ├── cross-review/            # 跨文件一致性验证（手动可选工具）
│   ├── biz-impact-analysis/     # 业务影响分析（5 步结构化）
│   ├── knowledge-audit/         # 知识库健康度审计
│   ├── knowledge-track/         # 文档利用追踪
│   ├── knowledge-repomap/       # 代码结构索引自动生成
│   └── workspace/               # 多仓库工作区（6 Phase 协调器）
├── agents/                      # 7 个 Agent 定义
...
│   ├── cost-tracker.py          # Token 成本追踪和预算告警
│   ├── gateguard-fact-force.py  # Fact-Forcing Gate 事实溯源合规检查
│   ├── marker_utils.py          # 共享幂等标记更新工具库
│   └── ecw_config.py            # 共享 ECW 配置读取工具库
├── templates/                   # 配置和知识文件模板
│   ├── ecw.yml                  # 项目配置模板
...
│   ├── workspace.yml            # 工作区配置模板
│   ├── workspace-claude.md      # 工作区 CLAUDE.md 模板
│   ├── cross-service-plan.md    # 跨服务变更方案模板
│   ├── service-task.md          # 单服务任务模板
│   ├── knowledge/               # 知识文件模板
│   │   ├── common/              # 跨域公共知识（6 个文件）
│   │   └── domain/              # 域级知识（3 个文件）
│   └── rules/                   # 工程规则模板
...
│   ├── check-freshness.sh       # 检测知识库文档中失效的 Java 类引用
│   ├── generate-repo-map.sh     # 自动提取组件签名生成 Repo Map
│   └── README.md                # 扫描输出格式规范
├── tests/                       # 三层测试套件
│   ├── Makefile                 # lint / test-hook / eval-* 目标
│   ├── static/                  # Layer 1：Python 静态扫描（21 项）+ pytest 单元测试（700 用例）
