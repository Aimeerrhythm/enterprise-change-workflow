...
后续路由：{完整路由链，如 ecw:domain-collab(多域协作) → Phase 2 → writing-plans → 实现 → ecw:biz-impact → Phase 3}
```
...
| **适用** | P0/P1（必做）、P2（建议做）、紧急通道（事后 biz-impact 时一并执行） |
| **不适用** | P3（不做 biz-impact，无 Phase 3） |
