...
| 风险等级 | 下游 skill |
|---------|-----------|
| P0（极高）| → `ecw:requirements-elicitation` → **Phase 2** → `writing-plans` → `ecw:spec-challenge` → 实现 → `ecw:cross-review` → `ecw:biz-impact-analysis` → **Phase 3** |
| P1（高） | → `ecw:requirements-elicitation` → **Phase 2** → `writing-plans` → 实现 → `ecw:cross-review` → `ecw:biz-impact-analysis` → **Phase 3** |
...
| 风险等级 | 下游 skill |
|---------|-----------|
| P0（极高）| → `ecw:domain-collab`（多域协作）→ **Phase 2** → `writing-plans` → `ecw:spec-challenge` → 实现 → `ecw:cross-review` → `ecw:biz-impact-analysis` → **Phase 3** |
| P1（高） | → `ecw:domain-collab`（多域协作）→ **Phase 2** → `writing-plans` → `ecw:spec-challenge` → 实现 → `ecw:cross-review` → `ecw:biz-impact-analysis` → **Phase 3** |
| P2（中） | → `ecw:domain-collab`（多域协作）→ **Phase 2** → `writing-plans` → 实现 → `ecw:cross-review` → `ecw:biz-impact-analysis`（建议）→ **Phase 3** |
...
| 风险等级 | 下游 skill |
|---------|-----------|
| 任何等级 | → invoke `superpowers:systematic-debugging`（定位 + 修复）→ mvn test → `ecw:cross-review` → ecw:biz-impact-analysis（P0/P1）→ **Phase 3** |
...
后续路由：{完整路由链，如 ecw:domain-collab(多域协作) → Phase 2 → writing-plans → 实现 → ecw:biz-impact-analysis → Phase 3}
```
...
| CR 完成后忘了跑 ecw:biz-impact-analysis | 代码变更的业务影响未评估 | P0/P1 变更 CR 完成后必须调用 `/biz-impact-analysis` |
