...
TDD:RED 阶段触发时，invoke `superpowers:test-driven-development` skill 执行 Red-Green-Refactor 循环。ECW 只负责在正确的时机触发，具体的 TDD 方法论由 superpowers skill 提供。
...
| 风险等级 | 下游 skill |
|---------|-----------|
| P0（极高）| → `ecw:requirements-elicitation` → **Phase 2** → `superpowers:writing-plans` → `ecw:spec-challenge` → **TDD:RED** → 实现(GREEN) → `ecw:impl-verify` → `ecw:biz-impact-analysis` → **Phase 3** |
| P1（高） | → `ecw:requirements-elicitation` → **Phase 2** → `superpowers:writing-plans` → **TDD:RED** → 实现(GREEN) → `ecw:impl-verify` → `ecw:biz-impact-analysis` → **Phase 3** |
| P2（中） | → `superpowers:writing-plans` → **TDD:RED** → 实现(GREEN) → `ecw:impl-verify` |
...
| 风险等级 | 下游 skill |
|---------|-----------|
| P0（极高）| → `ecw:domain-collab`（多域协作）→ **Phase 2** → `superpowers:writing-plans` → `ecw:spec-challenge` → **TDD:RED** → 实现(GREEN) → `ecw:impl-verify` → `ecw:biz-impact-analysis` → **Phase 3** |
| P1（高） | → `ecw:domain-collab`（多域协作）→ **Phase 2** → `superpowers:writing-plans` → `ecw:spec-challenge` → **TDD:RED** → 实现(GREEN) → `ecw:impl-verify` → `ecw:biz-impact-analysis` → **Phase 3** |
| P2（中） | → `ecw:domain-collab`（多域协作）→ **Phase 2** → `superpowers:writing-plans` → **TDD:RED** → 实现(GREEN) → `ecw:impl-verify` → `ecw:biz-impact-analysis`（建议）→ **Phase 3** |
...
| 风险等级 | 下游 skill |
|---------|-----------|
| 任何等级 | → invoke `superpowers:systematic-debugging`（定位根因）→ **TDD:RED**（写复现测试）→ 修复(GREEN) → mvn test → `ecw:impl-verify` → ecw:biz-impact-analysis（P0/P1）→ **Phase 3** |
...
后续路由：{完整路由链，如 ecw:domain-collab(多域协作) → Phase 2 → writing-plans → TDD:RED → 实现(GREEN) → ecw:biz-impact-analysis → Phase 3}
```
...
- **实现策略**: TBD（writing-plans 完成后根据 Task 数量确定）
...
核心要点：保留 Phase 1 记录等级 → 1 轮简化确认 → 精简 plan → 实现 + mvn test → 事后补做 ecw:biz-impact-analysis（标注 `[紧急通道]`）。
...
ecw:biz-impact-analysis 报告产出后自动执行。仅当当前会话中 Phase 1 或 Phase 2 产出了风险等级时触发。
