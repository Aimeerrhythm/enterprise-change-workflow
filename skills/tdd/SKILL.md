...
> **Downstream Handoff**: After the final Plan Task's GREEN phase completes, update `next` field (YAML key) **within the `<!-- ECW:STATUS:START/END -->` marker block** in session-state.md to `ecw:impl-verify`, then invoke `ecw:impl-verify`. If a pending `ecw:impl-verify` Task exists in TaskList, mark it `in_progress` first. If `auto_continue` field is missing or `no` in session-state.md, wait for user confirmation (backward compatibility).
