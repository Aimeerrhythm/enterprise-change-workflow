...
3. **Per-round immediate persistence**: As soon as each Round subagent returns, immediately append its results (with round header) to `.claude/ecw/session-data/{workflow-id}/impl-verify-findings.md`. Do NOT wait for all rounds to complete before persisting — each round is appended independently. This ensures completed round results survive even if subsequent rounds fail or context is compacted mid-verification.
4. Deduplicate findings across Rounds
5. **Finalize findings**: Before creating any fix Tasks or presenting to user, ensure `impl-verify-findings.md` is complete and deduplicated. Format per finding: severity, Round, file:line, description, expected vs actual, fix suggestion. After each re-verification round, append new findings and mark fixed ones as `[FIXED]`. This ensures a complete view survives context compaction.
...
3. Append each re-verification round result to `impl-verify-findings.md` (with round header and dimension label), marking previously reported findings for that dimension as `[FIXED]` or `[STILL-OPEN]`.
