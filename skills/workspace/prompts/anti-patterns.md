...
- **Never let child sessions write the coordinator-owned workflow state** — coordinator is the sole owner of the workspace-root file `{workspace}/.claude/ecw/session-data/{wf-id}/session-state.json`. Child sessions must NOT touch that path. They write their own artifacts under `{service}/.claude/ecw/session-data/{wf-id}/` — including a child-scoped `session-state.json` owned by that service's risk-classifier (the two session-state.json files live at different paths and never alias). Workspace-level state updates happen only at coordinator gate transitions.
- **Never skip Phase gates** — artifact must exist before next Phase
- **Never use code-reading tools during initial decomposition (Phase 1)** — The initial decomposition information source is workspace.yml requirement ONLY. No Read, Bash, Glob, Grep, or Explore tools. If code detail is needed to answer a question, it's an Open Question for Phase 2, not something to resolve during the Phase 1 initial decomposition.
- **Never put code-level detail in initial decomposition output** — class names, method names, field names, SQL in cross-service-plan.md or workspace-analysis-task.md are initial decomposition violations.
...
|-------------|---------|
| "I'll just quickly scan the code during initial decomposition to confirm the interaction pattern" | The initial decomposition is information-constrained: workspace.yml only. If you can't determine the pattern from the requirement, mark it "unclear" and let Phase 2 investigate. Scanning code during initial decomposition produces class-level detail that belongs to child sessions — and your scan will be incomplete anyway. |
...
| "The business decomposition is obvious, skip initial decomposition confirmation" | Interaction patterns (Dubbo vs MQ, sync vs async) are often ambiguous. Get user confirmation before Phase 2 starts. |
