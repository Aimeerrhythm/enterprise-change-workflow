...
- **Never let child sessions write coordinator-owned workflow state** — coordinator is the sole owner of `.claude/ecw/session-data/{wf-id}/session-state.json`. Child sessions only write their own artifacts (analysis-report.md, status.json). State updates are always done by the coordinator at gate transitions.
