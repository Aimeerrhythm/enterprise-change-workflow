...
| 文件 | 说明 |
|------|------|
| `cross-domain-rules.md` | 跨域调用规则与全局约束 |
...
1. **断裂引用检查** — 本次修改的文件中是否引用了不存在的 `.claude/` 路径
2. **残留引用检查** — 本次删除的文件是否还被其他文件引用
3. **Java 编译检查** — 修改了 `.java` 文件时自动执行 `mvn compile`，编译失败阻止完成

**定向提醒（不阻止，注入到 systemMessage）：**

4. **知识文档同步提醒** — 检测业务代码变更但对应域知识文档未更新，列出具体域和文件路径

技术检查失败会阻止任务标记为完成，强制修复后重试。技术检查通过后，会提醒执行 `ecw:cross-review` 交叉一致性验证 + 知识文档定向提醒。
