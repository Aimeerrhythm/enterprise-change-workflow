...
> 本文件属于 ECW 路由/治理元数据，用于 Agent 域定位与协作提示生成，不承载业务规则正文。
...
<!-- 
  ...
  **职责:** 一句话描述此域的业务范围和核心链路。
  ...
  5. 同步在 path-mappings.md 中添加代码路径→域映射
...
| 文件路径 | 说明 |
|---------|------|
| `.claude/knowledge/shared/cross-domain-calls.md` | 域间直接调用矩阵 |
| `.claude/knowledge/shared/mq-topology.md` | MQ Topic 发布/消费关系 |
| `.claude/knowledge/shared/shared-resources.md` | 被 2+ 域使用的共享资源 |
| `.claude/knowledge/shared/external-systems.md` | 外部系统集成 |
| `.claude/knowledge/shared/e2e-paths.md` | 端到端关键链路 |
